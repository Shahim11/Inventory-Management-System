...
class salesClass:
    def __init__(self, root):
        ...
        lbl_invoice = Label(self.root, text="Invoice No.", font=(
            "goudy old style", 15), bg="white").place(x=50, y=100)
        
        # ==Search Text==
        txt_invoice = Entry(self.root, textvariable=self.var_invoice, font=(
            "goudy old style", 15), bd=3, relief=RIDGE, bg="lightyellow").place(x=180, y=100, width=180, height=28)
 
        # ==Search Button==
        btn_search = Button(self.root, text="Search", command=self.Search, font=(
            "times new roman", 15, "bold"), cursor="hand2", bg="#2196f3", fg="white").place(x=370, y=100, width=120, height=28)
        
        # ==Clear Button==
        btn_clear = Button(self.root, text="Clear", command=self.clear, font=("times new roman", 15, "bold"),
                           cursor="hand2", bg="lightgray", fg="black").place(x=500, y=100, width=120, height=28)
        ...
        lbl_title2 = Label(bill_frame, text="Customer Bill Area", font=(
            "goudy old style", 20), bg="orange", fg="white", bd=3, relief=RIDGE).pack(side=TOP, fill=X)
