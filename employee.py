...
class employeeClass:
    def __init__(self, root):
        ...
        self.var_doj = StringVar()        # Date of Join
        ...
        # ==Options==
        cmb_search = ttk.Combobox(SearchFrame, values=("Select", "Email", "Name", "Contact"),
                                  textvariable=self.var_searchby, state="readonly", justify=CENTER, font=("goudy old style", 15))
        cmb_search.place(x=10,y=10,width=180)
        cmb_search.current(0)

        # ==Search Text==
        txt_search = Entry(SearchFrame, font=("goudy old style", 15),
                           textvariable=self.var_searchtxt, bg="lightyellow").place(x=200, y=10)
        
        # ==Search Button==
        btn_search = Button(SearchFrame, text="Search", command=self.search, font=(
            "goudy old style", 15), bg="#4caf50", fg="white", cursor="hand2").place(x=420, y=8, width=150, height=30)
        ...
        txt_empid = Entry(self.root, textvariable=self.var_emp_id, bg="lightyellow").place(
            x=150, y=150, width=180, height=27)
        ...
        txt_contact=Entry(self.root,textvariable=self.var_contact,bg="lightyellow").place(x=850,y=150,width=180,height=27)
        ...
        txt_name = Entry(self.root, textvariable=self.var_name, bg="lightyellow").place(
            x=150, y=190, width=180, height=27)
        
        txt_dob=Entry(self.root,textvariable=self.var_dob,bg="lightyellow").place(x=500,y=190,width=180,height=27)
        
        txt_doj=Entry(self.root,textvariable=self.var_doj,bg="lightyellow").place(x=850,y=190,width=180,height=27)
        ...
        txt_email=Entry(self.root,textvariable=self.var_email,bg="lightyellow").place(x=150,y=230,width=180,height=27)
        
        txt_pass=Entry(self.root,textvariable=self.var_pass,bg="lightyellow").place(x=500,y=230,width=180,height=27)
        
        cmb_utype = ttk.Combobox(self.root, textvariable=self.var_utype, values=(
            "Select", "Admin", "Employee"), state="readonly", justify=CENTER, font=("goudy old style", 15))
        cmb_utype.place(x=850,y=230,width=180,height=27)
        cmb_utype.current(0)
        ...
        btn_add = Button(self.root, text="Save", command=self.add, bg="#2196f3", font=(
            "goudy old style", 15), fg="white", cursor="hand2").place(x=500, y=305, width=110, height=28)
      
        btn_update = Button(self.root, text="Update", command=self.update, bg="#4caf50", font=(
            "goudy old style", 15), fg="white", cursor="hand2").place(x=620, y=305, width=110, height=28)
       
        btn_delete = Button(self.root, text="Delete", command=self.delete, bg="#f44336", font=(
            "goudy old style", 15), fg="white", cursor="hand2").place(x=740, y=305, width=110, height=28)
       
        btn_clear = Button(self.root, text="Clear", command=self.clear,bg="#607d8b", font=(
            "goudy old style", 15), fg="white", cursor="hand2").place(x=860, y=305, width=110, height=28)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                if row != None:
                    messagebox.showerror(
                        "Error", "This Employee Id is already assigned, try different one.")
                else:
            ...
            if self.var_emp_id.get() == "":
                ...
                if row == None:
                    ...
                    if op == True:
                        ...
                        con.commit()
                        messagebox.showinfo(
                            "Success", "Employee Deleted Successfully", parent=self.root)
                        self.clear()
